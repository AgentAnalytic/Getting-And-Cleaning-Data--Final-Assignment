...
for (i in 1:l) {
  features[i, 3] <- paste(features[i, 1], "-", features[i, 2])
}
...
data_mean_std <-
  dplyr::rename(data_mean_std, "activity_code" = "V1")
...
data_mean_std <-
  dplyr::rename(data_mean_std, "subject_person_ID" = "V1")


#giving activity_label a descriptive activity names to name the activities in the data set

activity_label <-
  dplyr::rename(activity_label, "activity_type" = "V2")
activity_label <-
  dplyr::rename(activity_label, "activity_code" = "V1")
...
TidyData <-
  data_set %>% group_by(ActivityType, SubjectPersonId) %>% summarise_each(funs = "mean")
